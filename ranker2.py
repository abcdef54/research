...
EXPERIMENT 3B (Can Tournament Ranking Beat One-Shot Ranking?) — ADDED, fully config-flag gated.
  Supersedes Experiment 3A: same node and `rank_mode` strings, but the round grouping changed (see
  `_tournament_groups`). MOTIVATION: Exp 2 found one-shot N-way ranking works at Width=3 but DEGRADES
  at Width=5 — the ranker saturates comparing too many candidates at once — and that N=3 is its sweet
  spot. 3A then over-corrected: its power-of-2 split forced the larger widths into PAIRWISE brackets
  (N=8 -> [2,2,2,2], ~7 ranking calls), and the deep chain of 2-way decisions accumulated selector
  error (Extra: 84.84% final vs 95.30% oracle). 3B keeps the SAME generator and SAME one-shot rankers
  and changes ONLY how they are CALLED: each round ranks floor(n/3) groups of at most MAX_RANK_GROUP
  (=3) and BYES the remainder, hitting the minimum ceil((n-1)/2) rank calls. Pure orchestration — no new prompt, no
  regeneration, no verifier/critic, no abstention, no extra generation round. Driven by `rank_mode`:
    - "tournament_no_reasoning"   -> `tournament_select` node, per-group rank_no_reasoning  (3B-no)
    - "tournament_with_reasoning" -> `tournament_select` node, per-group rank_with_reasoning (3B-rsn)
  Tournament uses ODD benchmark widths (TOURNAMENT_MODE_CONFIG, Experiment 3B): low=1, medium=3,
  high=5, extra=7, max=9. Each round ranks floor(n/3) groups of 3 and byes the trailing n%3 so the
  survivor count drops straight to <=3 and finishes in one final call: 3 -> [3] (1 call); 5 -> [3]
  + 2 byes (2 calls); 7 -> [3,3] + 1 bye (3 calls); 9 -> [3,3,3] (4 calls). (Experiment 3A used even widths with a power-of-2 split,
  which degraded into pairwise brackets — N=8 -> [2,2,2,2], 7 calls — and accumulated selector
  error; 3B replaces that grouping.) Adds tournament_rounds / tournament_rank_calls /
...
# Experiment 3B: a single tournament ranking call may compare AT MOST this many candidates. Exp 2
# found one-shot ranking degrades as candidate count grows AND that N=3 is the ranker's sweet spot,
# so each round ranks groups of 3 and byes the remainder (minimum rank calls). Value is 3
# (unchanged from 3A — 3A's pairwise brackets came from its power-of-2 group COUNT, not this cap).
# Distinct from the PARKED RANK_GROUP above (that one is for the old iterative ranker).
MAX_RANK_GROUP = 3
...
class AgentState(TypedDict):
    ...
    # selector (Experiment 3B — hierarchical tournament). Populated ONLY by `tournament_select`;
...
# Experiment 3B: tournament ranking uses ODD widths (1/3/5/7/9); each round packs into groups of up to 3
...
TOURNAMENT_MODE_CONFIG = {
    ...
    "medium": {"max_width": 3, "max_iterations": 0, "max_budget": 3},   # 3B: [3] one-shot (1 call)
    "high":   {"max_width": 5, "max_iterations": 0, "max_budget": 5},   # 3B: [3] + 2 byes, then 3-way final (2 calls)
    "extra":  {"max_width": 7, "max_iterations": 0, "max_budget": 7},   # 3B: [3,3] + 1 bye, then 3-way final (3 calls)
    "max":    {"max_width": 9, "max_iterations": 0, "max_budget": 9},   # 3B: [3,3,3], then 3-way final (4 calls)
}
...
def get_config(mode: str, steps: str, bypass_governor: bool = False, tournament: bool = False) -> Dict:
    ...
    tournament=True selects TOURNAMENT_MODE_CONFIG (Experiment 3B odd widths 1/3/5/7/9) instead of
    MODE_CONFIG; the two tables share the same mode keys, so every other code path is unaffected."""
...
def setup(state: AgentState, config: RunnableConfig) -> dict:
    ...
    # Experiment 3B: tournament modes use the ODD-width table (min-call grouping: rank 3s, bye remainder); all other modes
    ...
    return {
        ...
        "pool": [],
        "best": None,
        # Experiment 2/3B selector flag (default "majority" => Experiment-1 path is unchanged).
        "rank_mode": rank_mode,
    }
...
# ───────────────────── Experiment 3B selector (ACTIVE — hierarchical tournament) ─────────────────────
...
def _tournament_groups(candidates: list["Candidate"]) -> list[list["Candidate"]]:
    """One round's bracket split for Experiment 3B — the MINIMUM-rank-call rule. Rank floor(n/3) full
...
async def tournament_rank(
    ...
    """Experiment 3B. Hierarchical tournament selector — an ORCHESTRATION layer over the existing
    ...
    trailing n%3 (minimum rank calls) -> rank each group with the chosen one-shot ranker -> advance
    winners (a byed singleton carries over with no call) -> recurse until one candidate remains.
...
async def tournament_select(state: AgentState, config: RunnableConfig) -> dict:
    """Experiment 3B selector node. Generation already produced `pool`; here we select Top-1 via a
    ...
    print(f"\n[IDX {idx}] ============ Tournament-Select Node (Experiment 3B) ============\n")
    print(f"[IDX {idx}] Rank mode: {rank_mode}")
    print(f"[IDX {idx}] Candidates: {len(pool)}")
...
def route_selector(state: AgentState) -> Literal["finalize", "rank_select", "tournament_select"]:
    """Selector router. Config flag `rank_mode` (resolved into state by `setup`) chooses the selector:
      - "tournament_no_reasoning" / "tournament_with_reasoning" -> `tournament_select` (Exp 3B bracket)
...
builder.add_node("tournament_select", tournament_select)  # Experiment 3B selector: hierarchical tournament
...
# ranking (Exp 3B) diverts to tournament_select. The first two selectors are unchanged.
...
if __name__ == "__main__":
    config = {
        "configurable": {
            ...
            # Experiment 3B (hierarchical tournament; ODD widths 1/3/5/7/9, min-call grouping):
            #   "rank_mode": "tournament_no_reasoning",   # 3B: per-group rank_no_reasoning
            #   "rank_mode": "tournament_with_reasoning", # 3B: per-group rank_with_reasoning
